...
func InitTemplate(templateDir string) (HtmlTemplate, error) {
	tp, err := readTemplate([]string{"index", "category", "custom", "details", "login", "pigeonhole", "writing"}, templateDir)
...
func readTemplate(templates []string, templateDir string) ([]TemplateBlog, error) {
	...
	for _, view := range templates {
		...
		t.Funcs(template.FuncMap{
			...
			"date":        Date,
			"dateDay":     DateDay})
		t, err := t.ParseFiles(_index, home, header, footer, personal, post, pagination)
		if err != nil {
			log.Printf("模板解析失败ParseFiles error: %v", err)
			return nil, err
		}
